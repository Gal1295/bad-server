...
export const uploadFile = async (
    ...
    const { size, filename, originalname, mimetype } = req.file
    const originalNameWithoutExt = originalname.replace(/\.[^/.]+$/, "")
    if (filename.includes(originalNameWithoutExt)) {
        return next(new BadRequestError('Оригинальное имя файла не должно использоваться в пути'))
    }
